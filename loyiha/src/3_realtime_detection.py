...
class TTSEngine:
    ...
    def _path(self, text):
        ...
        # Oldin .wav versiyasini tekshir (oldindan yozilgan)
        wav = os.path.join(AUDIO_DIR, f"{safe}_{self.lang}.wav")
    ...
    def _synthesize(self, text):
        ...
        if not os.path.exists(path):
            try:
                ...
                safe = "".join(c if c.isalnum() else "_" for c in text)
                path = os.path.join(AUDIO_DIR, f"{safe}_{self.lang}.mp3")
...
class Predictor:
    ...
    def __init__(self, window=12, conf_thresh=0.72):
        self.WINDOW      = window
        ...
        print(f"[OK] Model yuklandi: {self.config.get('model_name', '—')}")
...
def main():
    ...
    cfg = load_settings()
    print(f"[OK] Sozlamalar yuklandi: til={cfg['lang']}, chegara={cfg['conf_thr']}")

    # Model va audio yuklanishi
    predictor  = Predictor(window=cfg["window_size"], conf_thresh=cfg["conf_thr"])
    tts        = TTSEngine(lang=cfg["lang"])
